...
$(document).on('click', '[data-bs-toggle="dropdown"]', function (e) {
    ...
    recibosHtml = pagamento.recibos.map(function (r) {
        return `<tr>
                    <td>
                        <a href="#" class="open-modal font-w600"
                            data-url="${r.blob || ''}"
                            data-nome="${r.nome_arquivo || ''}">
                            <strong>${r.nome_arquivo || ''}</strong>
                        </a>
                    </td>
                    <td>
                        <i class="fa fa-trash text-danger deleteFileId"
                            data-recibo-id="${r.id}"
                            data-arquivo="${r.nome_arquivo}"
                            data-toggle="tooltip"
                            data-placement="top"
                            title="Excluir"></i>
                    </td>
